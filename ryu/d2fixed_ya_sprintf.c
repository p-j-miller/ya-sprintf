// Copyright 2018 Ulf Adams (except for new Peter Miller functions which are Copyright Peter Miller 2026).
//
...
// This version adds some new functions to interface with ya_sprintf - changes by Peter Miller (PMi) 1/2026
...
// PMi - looking at its use the return value should be the length of the string
#if defined(_UCRT) // PMi this behaviour is only seen when using the UCRT - its not linked to using the MSC compiler 
  // TODO: Check that -nan is expected output on Windows.
  if (sign) {
    result[0] = '-';
  }
  if (mantissa) { // PMi added this case in to match UCRT
    if( sign && mantissa== (1ull << (DOUBLE_MANTISSA_BITS - 1))) { 
      memcpy(result + sign, "nan(ind)", 8);
      return sign + 8;
    }          
...
#endif
  memcpy(result + sign, "inf", 3); // PMi most systems produce "inf" rather than "Infinity" 
  return sign + 3;// PMi - was 8 but I changed the string to "inf"
}
...
char* d2fixed(double d, uint32_t precision) {
  ...
  if(buffer==NULL) return NULL; // PMi 
...
/* ************************************************************************************************************************************************ */
/*																																					*/
/* Functions below were created by Peter Miller for use with ya_sprintf - they are heavily based on the original ryu functions above				*/
/* They assume a standard "double" is supplied (as an already split up mantissa/exponent) 															*/
/* they do not check for nan, inf or zero as these are all treated as special cases in ya_sprintf													*/
/*																																					*/
/* These are Copyright (C) Peter Miller 2026.																										*/
/*																																					*/
/* ************************************************************************************************************************************************ */
...
//   10^(olength-1) <= digits < 10^olength
// e.g., by passing `olength` as `decimalLength9(digits)`.
// note that in this version [ append_d_digits_ya_sprintf() ]the decimal point is not actually stored in the string
static inline void append_d_digits_ya_sprintf(const uint32_t olength, uint32_t digits, char* const result) {
#ifdef RYU_DEBUG
  printf("DIGITS=%u\n", digits);
#endif
  uint32_t i = 0;
...
// main double->string conversion function
...
int d2exp_buffered_n_ya_sprintf(const uint64_t ieeeMantissa,const uint32_t ieeeExponent, uint32_t precision, char* result,int32_t *decimal_pos) 
{
...
#ifdef RYU_DEBUG
  printf("d2exp_buffered_n_ya_sprintf() -> %" PRIu64 " * 2^%d\n", m2, e2);
#endif
  ...
  if (e2 >= -52) {
    ...
    for (int32_t i = len - 1; i >= 0; --i) {
      ...
      if (printedDigits != 0) {
        ...
        if (printDecimalPoint) {
          append_d_digits_ya_sprintf(availableDigits, digits, result + index);
          index += availableDigits ; // no decimal point
        } else {
          result[index++] = (char) ('0' + digits);
        }
        printedDigits = availableDigits;
  ...
  if (printedDigits != 0) {
    ...
    if (printDecimalPoint) {
      append_d_digits_ya_sprintf(maximum, digits, result + index);
      index += maximum ; // no decimal point
    } else {
      result[index++] = (char) ('0' + digits);
    }
  }
