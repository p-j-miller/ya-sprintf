...
// This file has minor changes by Peter Miller, 2026 - marked "PMi".
...
// PMi 	- it's still slightly faster to use 64bit intrinsics than __int128 on X64, so update the logic below to make this happen independent of the MSC compiler 
// 		- intrinsics are available on gcc/WinLibs and clang as well as MSC so no need to check for the compiler
#if defined(__SIZEOF_INT128__) && !defined(_M_X64) && !defined(RYU_ONLY_64_BIT_OPS) && !defined(__BORLANDC__)
#define HAS_UINT128
#elif  !defined(RYU_ONLY_64_BIT_OPS) && defined(_M_X64) && (!defined(__BORLANDC__) || (defined(__BORLANDC__) && defined(_UCRT)))
...
static inline uint64_t shiftright128(const uint64_t lo, const uint64_t hi, const uint32_t dist) {
  ...
  // PMi: with s2d_fast_atof.c 70 was seen so function has been adjusted to allow shifts up to 127
#ifdef RYU_DEBUG
  static uint32_t max_dist=0;
  if(dist>=64) 
	{if(dist>max_dist) max_dist=dist;
	 printf("shiftright128 hi=0X%zx lo=0X%zx dist=%u (0X%x), max dist so far=%u (0x%x)\n",hi,lo,dist,dist,max_dist,max_dist);
	}
#endif
#if 1 /* PMi new version, allows 0..127 shift */
  assert(dist < 128);
  if(dist>=64)
	return __shiftright128(hi, 0, (unsigned char) (dist-64));
  else
	return __shiftright128(lo, hi, (unsigned char) dist);
#else /* original version limited 0..63 */
  assert(dist < 64);
  return __shiftright128(lo, hi, (unsigned char) dist);
#endif  
}
...
static inline uint64_t shiftright128(const uint64_t lo, const uint64_t hi, const uint32_t dist) {
#if 1 /* PMi - allow greater than 64 shift - see above */
  assert(dist < 128);
  assert(dist > 0);
  if(dist>=64)
	  return hi>>(dist-64);
   else return (hi << (64 - dist)) | (lo >> dist);
#else
