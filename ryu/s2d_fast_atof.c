// Copyright 2019 Ulf Adams (except for new Peter Miller functions which are Copyright Peter Miller 2026).
//
...
// This version adds some new functions to interface with fast_atof - changes by Peter Miller (PMi) 1/2026

//#define RYU_DEBUG /* define to get lots of debigging printf's */
...
// This is heavily based on the code in s2d_n() above
double ryu_conv_mant_exp_to_double(bool signedM,uint64_t m10,int32_t e10)
{
  ...
  bool trailingZeros;
#ifdef RYU_DEBUG
  printf("Input: m10 * 10^e10 = %" PRIu64 " * 10^%d\n", m10, e10);
#endif  
  if (m10 == 0) 
	{// zero
     uint64_t ieee = ((uint64_t) signedM) << (DOUBLE_EXPONENT_BITS + DOUBLE_MANTISSA_BITS);
  ...
  if(e10==0)
 	{// special case [but likely quite common], exp==0, we can directly convert uint64_t to double via a cast
 	 if(signedM) return -((double)m10); // m10 is unsigned so cannot do -m10 !
 	 else return (double) m10;
	}	  
  else if (e10 > 0) 
	{
     // The length of m * 10^e in bits is:
     ...
     m2 = mulShift64(m10, pow5, j);
#else
	 if (e10 >= DOUBLE_POW5_TABLE_SIZE) // was assert(e10 < DOUBLE_POW5_TABLE_SIZE);
	...
	{// e10<0
     e2 = floor_log2(m10) + e10 - ceil_log2pow5(-e10) - (DOUBLE_MANTISSA_BITS + 1);
     int j = e2 - e10 + ceil_log2pow5(-e10) - 1 + DOUBLE_POW5_INV_BITCOUNT;
     if (-e10 >= DOUBLE_POW5_INV_TABLE_SIZE)  
     	{
		 // DOUBLE_POW5_INV_TABLE_SIZE=342
		 // 10^19 will fit into a uint64, and DBL_DENORM_MIN = 4.94065645841246544176568792868221372e-324 so could have a valid 10's exponent of -(324+19)=-343
		 // rather than extend the table, here we fix things up by dividing mant by 10 and increasing exponent until exponent is in range of mantissa is 0
		 int m10_rem=0;
		 while(m10>0 && -e10 >= DOUBLE_POW5_INV_TABLE_SIZE)  
		 	{m10_rem=m10%10; // keep last digit for rounding
			 m10/=10;
		 	 e10++;
		 	}
		 if (-e10 >= DOUBLE_POW5_INV_TABLE_SIZE || m10==0)
		 	{// number is really too small, round down to 0; return +/-0.0.
		 	 uint64_t ieee = ((uint64_t) signedM) << (DOUBLE_EXPONENT_BITS + DOUBLE_MANTISSA_BITS);
		 	 return int64Bits2Double(ieee);
		 	}
		 if(m10_rem>=5) m10++; // round m10 based on last digit lost, not sure if this is essential?
		 // m10 & e10 have changed recalculate e2 & j to reflect this
		 e2 = floor_log2(m10) + e10 - ceil_log2pow5(-e10) - (DOUBLE_MANTISSA_BITS + 1);
     	 j = e2 - e10 + ceil_log2pow5(-e10) - 1 + DOUBLE_POW5_INV_BITCOUNT;	
		}     
#if defined(RYU_OPTIMIZE_SIZE)
     uint64_t pow5[2];
     double_computeInvPow5(-e10, pow5);
